...
- **React 19**, **TypeScript**
- **Tailwind CSS** + **Framer Motion** (scroll reveals, hero animations)
...
The frontend reads site-wide contact/social info (email, phone, WhatsApp, Instagram, …) from the Strapi single type **Personal Informations**:
...
- `src/components/site/` — global components: `Header`, `Footer`, `WhatsAppButton`, `EnquiryForm`, etc.
- `src/components/ui/` — shadcn-style UI primitives
- `src/lib/site.ts` — Strapi connectivity (URL resolution + fetch)
- `src/lib/data/` — static content (projects, blog, destinations, etc.)
